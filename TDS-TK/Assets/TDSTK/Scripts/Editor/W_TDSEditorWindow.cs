...
namespace TDSTK {
	...
	public class TDSEditorWindow : EditorWindow {
		...
		protected Vector2 DrawUnitBaseStats(float startX, float startY, Unit unit, bool isPlayer){
			...
			EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
			unit.hitPointFull=EditorGUI.DelayedFloatField(new Rect(startX+spaceX, startY, 40, height), unit.hitPointFull);
			...
			EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
			unit.hpRegenRate=EditorGUI.DelayedFloatField(new Rect(startX+spaceX, startY, 40, height), unit.hpRegenRate);
			...
			EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
			if(unit.hpRegenRate>0) unit.hpRegenStagger=EditorGUI.DelayedFloatField(new Rect(startX+spaceX, startY, 40, height), unit.hpRegenStagger);
			...
			if(isPlayer){
				...
				EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
				unit.energyFull=EditorGUI.DelayedFloatField(new Rect(startX+spaceX, startY, 40, height), unit.energyFull);
				...
				EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
				unit.energyRate=EditorGUI.DelayedFloatField(new Rect(startX+spaceX, startY, 40, height), unit.energyRate);
			}
		...
		*/
		...
		protected Vector2 DrawAttackStats1(float startX, float startY, AttackStats aStats, bool showAOE=true, bool showPhysics=true, string label="Attack Stats"){
			...
			cont=new GUIContent(lbSp+"Damage (Min/Max):", "Damage value done to the target's hit-point.");
			EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
			aStats.damageMin=EditorGUI.DelayedFloatField(new Rect(startX+spaceX, startY, 40, height), aStats.damageMin);
			aStats.damageMax=EditorGUI.DelayedFloatField(new Rect(startX+spaceX+42, startY, 40, height), aStats.damageMax);
			...
			if(showAOE){
				cont=new GUIContent(lbSp+"AOE Radius:", "Area of effect radius of the attack. Any hostile unit within the area is affected by the attack");
				EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
				aStats.aoeRadius=EditorGUI.DelayedFloatField(new Rect(startX+spaceX, startY, 40, height), aStats.aoeRadius);
			...
			cont=new GUIContent(lbSp+"Critical Chance:", "The chance of the attack to score a critical. Takes value from 0-1 with 0.3 being 30% to score a critical");
			EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
			aStats.critChance=EditorGUI.DelayedFloatField(new Rect(startX+spaceX, startY, 40, height), aStats.critChance);
			
			cont=new GUIContent(lbSp+"Critical Multiplier:", "The multiplier to be applied to damage if the attack scores a critical.\n - 1.5 for 150% of normal damage, 2 for 200% and so on");
			EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
			if(aStats.critChance>0) aStats.critMultiplier=EditorGUI.DelayedFloatField(new Rect(startX+spaceX, startY, 40, height), aStats.critMultiplier);
			else EditorGUI.LabelField(new Rect(startX+spaceX, startY, 40, height), "-");
			...
			if(showPhysics){
				cont=new GUIContent(lbSp+"Impact Force:", "If the attack will applies a knock back force to the target\nOnly applies if the attack is a direct hit from a shoot object");
				EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
				aStats.impactForce=EditorGUI.DelayedFloatField(new Rect(startX+spaceX, startY, 40, height), aStats.impactForce);
				
				cont=new GUIContent(lbSp+"Explosion Radius:", "The radius in which all unit is affected by explosion force");
				EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
				aStats.explosionRadius=EditorGUI.DelayedFloatField(new Rect(startX+spaceX, startY, 40, height), aStats.explosionRadius);
				
				cont=new GUIContent(lbSp+"Explosion Force:", "The force of the explosion which pushes all affect unit away from the impact point");
				EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
				if(aStats.explosionRadius>0) aStats.explosionForce=EditorGUI.DelayedFloatField(new Rect(startX+spaceX, startY, 40, height), aStats.explosionForce);
				else EditorGUI.LabelField(new Rect(startX+spaceX, startY, 40, height), "-");
			}
		...
		protected Vector2 DrawDestroyEffectObj(float startX, float startY, SerializedObject srlObj){
			...
			EditorGUI.LabelField(new Rect(startX, startY, width, height), cont);
			EditorGUI.PropertyField(new Rect(startX+spaceX, startY-10, width, height+10), srlObj.FindProperty("spawnImmunity"), contN);
		...
		public static void SetDamageDB(DamageTable_DB db, string[] dmgLabel, string[] armLabel){
			damageTableDB=db;
		...
		//protected static void UpdateLabel_DamageTable(){ TDSEditor.UpdateLabel_DamageTable(); }
		public static void SetProgressionStats(ProgressionStats_DB db){	//, string[] dmgLabel, string[] armLabel){
			progressDB=db;
			//damageTypeLabel=dmgLabel;
			//armorTypeLabel=armLabel;
		}
		...
		public static void SetCollectibleDB(Collectible_DB db, List<int> IDList, string[] label){
			collectibleDB=db;
		...
		public static void SetUnitAIDB(UnitAI_DB db, List<int> IDList, string[] label){
			unitAIDB=db;
			unitAIIDList=IDList;
			unitAILabel=label;
		}
