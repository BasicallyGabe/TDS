...
namespace TDSTK {
	...
	public class UnitPlayerEditorWindow : TDSEditorWindow {
		...
		Vector2 DrawUnitConfigurator(float startX, float startY, UnitPlayer unit){
			...
			EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
			if(unit.turretObj!=null) unit.enableTurretRotate=EditorGUI.Toggle(new Rect(startX+spaceX, startY, width, height), unit.enableTurretRotate);
			else EditorGUI.LabelField(new Rect(startX+spaceX, startY, width, height), "n/a (no turret)");
			
			cont=new GUIContent("Turret Aim Mode:", "The way in which the turret aim direction will be calculated");
			EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
			if(unit.turretObj==null) EditorGUI.LabelField(new Rect(startX+spaceX, startY, width, height), "n/a (no turret)");
			else if(unit.enableTurretRotate){
			...
			EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
			if(unit.turretObj==null) EditorGUI.LabelField(new Rect(startX+spaceX, startY, width, height), "n/a (no turret)");
			else if(unit.enableTurretRotate) EditorGUI.LabelField(new Rect(startX+spaceX, startY, width, height), "-");
			else unit.aimAtTravelDirection=EditorGUI.Toggle(new Rect(startX+spaceX, startY, width, height), unit.aimAtTravelDirection);
		...
		void SelectItem(){
			if(unitPlayerDB.unitList.Count<0) return;
