	...
	public class PerkEditorWindow : TDSEditorWindow {
		...
		private float DrawPerkGeneralSetting(float startX, float startY, Perk perk){
			...
			if(foldGeneral){
				startX+=15;
					...
					EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
					if(perk.repeatable) perk.limit=EditorGUI.DelayedIntField(new Rect(startX+spaceX, startY, widthS, height), perk.limit);
				...
				startY+=5;
					...
					EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
					perk.minLevel=EditorGUI.DelayedIntField(new Rect(startX+spaceX, startY, widthS, height), perk.minLevel);
					
					cont=new GUIContent("Min PerkPoint req:", "Minimum perk point to have before the perk becoming available");
					EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
					perk.minPerkPoint=EditorGUI.DelayedIntField(new Rect(startX+spaceX, startY, widthS, height), perk.minPerkPoint);
					
				startY+=5;
					
					cont=new GUIContent("Energy Cost:", "The energy cost required to purchase the perk");
					EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
					perk.energyCost=EditorGUI.DelayedFloatField(new Rect(startX+spaceX, startY, widthS, height), perk.energyCost);
					
					cont=new GUIContent("Cost:", "The perk currency required to purchase the perk");
					EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
					perk.cost=EditorGUI.DelayedIntField(new Rect(startX+spaceX, startY, widthS, height), perk.cost);
		...
		private float DrawGeneralStats(float startX, float startY, Perk perk){
			...
			string stackText="\nValue stacks with multiple perks";
				...
				cont=new GUIContent("Perk Currency:", "Perk currency gain - For perk that can be bought with energy");
				EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
				perk.perkCurrencyGain=EditorGUI.IntField(new Rect(startX+spaceX, startY, widthS, height), perk.perkCurrencyGain);
		...
		private float DrawPerkConfigurator(float startX, float startY, Perk perk){
			...
			perk.desp=EditorGUI.DelayedTextField(new Rect(startX, startY+spaceY-3, 270, 150), perk.desp, style);
