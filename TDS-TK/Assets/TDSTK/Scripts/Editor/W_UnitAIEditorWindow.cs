...
namespace TDSTK {
	...
	public class UnitAIEditorWindow : TDSEditorWindow {
		...
		public void OnGUI(){
			...
			if(unitList.Count==0 || srlObj==null) return;
		...
		Vector2 DrawUnitConfigurator(float startX, float startY, UnitAI unit){
			...
			if(showAttackSetting){
				...
				if(enableRangeAttack){
					...
					EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
					if(unit.turretObj!=null) unit.turretTrackingSpeed=EditorGUI.DelayedFloatField(new Rect(startX+spaceX, startY, 40, height), unit.turretTrackingSpeed);
					else EditorGUI.LabelField(new Rect(startX+spaceX, startY, 40, height), "-");
		...
		void SelectItem(){
			if(unitAIDB.unitList.Count<0) return;
