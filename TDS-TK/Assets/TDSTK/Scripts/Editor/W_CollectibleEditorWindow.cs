...
namespace TDSTK {
	...
	public class CollectibleEditorWindow : TDSEditorWindow {
		...
		public void OnGUI(){
			...
			if(collectibleList.Count==0) return;
			...
			scrollPos = GUI.BeginScrollView(visibleRect, scrollPos, contentRect);
				...
				if(EditorGUI.EndChangeCheck() && selectID>=0){
					...
					//GameObject collectibleObj=PrefabUtility.LoadPrefabContents(AssetDatabase.GetAssetPath(collectibleList[selectID].gameObject));
					//Collectible selectedItem=collectibleObj.GetComponent<Collectible>();
					//selectedItem=collectibleList[selectID];
					//GameObject obj=PrefabUtility.SavePrefabAsset(selectedItem.gameObject);
					
					string assetPath = AssetDatabase.GetAssetPath(collectibleList[selectID].gameObject);
					
					GameObject itemObj=PrefabUtility.LoadPrefabContents(assetPath);
					Collectible selectedObj=itemObj.GetComponent<Collectible>();
					
					EditorUtility.CopySerialized(collectibleList[selectID], selectedObj);
		...
		Vector2 DrawCollectibleConfigurator(float startX, float startY, Collectible cItem){
			...
			cItem.collectibleName=EditorGUI.DelayedTextField(new Rect(startX+spaceX-65, startY, width-5, height), cItem.collectibleName);
			...
			if(cItem.type==_CollectType.Ability){
				...
				EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
				cItem.life=EditorGUI.DelayedIntField(new Rect(startX+spaceX, startY, 40, height), cItem.life);
				
				startY+=10;
				
				cont=new GUIContent("HitPoint:", "The amount of hit-point gained by the player");
				EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
				cItem.hitPoint=EditorGUI.DelayedFloatField(new Rect(startX+spaceX, startY, 40, height), cItem.hitPoint);
				
				cont=new GUIContent("Energy:", "The amount of energy gained by the player");
				EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
				cItem.energy=EditorGUI.DelayedFloatField(new Rect(startX+spaceX, startY, 40, height), cItem.energy);
				
				startY+=10;
				
				//cont=new GUIContent("Credits:", "The amount of credist gained by the player");
				//EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
				//cItem.credit=EditorGUI.DelayedIntField(new Rect(startX+spaceX, startY, 40, height), cItem.credit);
				
				cont=new GUIContent("Score:", "The amount of points gained by the player");
				EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
				cItem.score=EditorGUI.DelayedIntField(new Rect(startX+spaceX, startY, 40, height), cItem.score);
				
				startY+=10;
				
				cont=new GUIContent("Ammo:", "The amount of ammo gained by the player. If set as -1, the ammo count will be refilled to full");
				EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
				cItem.ammo=EditorGUI.DelayedIntField(new Rect(startX+spaceX, startY, 40, height), cItem.ammo);
				...
				EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
				cItem.exp=EditorGUI.DelayedIntField(new Rect(startX+spaceX, startY, 40, height), cItem.exp);
				
				cont=new GUIContent("Perk Currency:", "The amount of perk currency gained by the player.");
				EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
				cItem.perkCurrency=EditorGUI.DelayedIntField(new Rect(startX+spaceX, startY, 40, height), cItem.perkCurrency);
				...
				if(cItem.gainWeapon){
					...
					if(cItem.weaponType==Collectible._WeaponType.Temporary)
						cItem.tempWeapDuration=EditorGUI.DelayedFloatField(new Rect(startX+spaceX, startY, 40, height), cItem.tempWeapDuration);
					else 
				...
				//if(cItem.type==_CollectType.AOEHostile){
					...
					//EditorGUI.LabelField(new Rect(startX, startY+=spaceY, width, height), cont);
					//cItem.aoeRange=EditorGUI.DelayedFloatField(new Rect(startX+spaceX, startY, 40, height), cItem.aoeRange);
				//}
			...
			if(cItem.triggerEffectObj!=null){
				...
				if(cItem.autoDestroyEffectObj){
					...
					cItem.effectObjActiveDuration=EditorGUI.DelayedFloatField(new Rect(startX+spaceX+width-58, startY, 40, height), cItem.effectObjActiveDuration);
				}
			}
			...
			if(cItem.selfDestruct)
				cItem.selfDestructDuration=EditorGUI.DelayedFloatField(new Rect(startX+spaceX, startY, 40, height), cItem.selfDestructDuration);
			else 
				EditorGUI.LabelField(new Rect(startX+spaceX, startY, 40, height), "-");
			...
			if(cItem.selfDestruct && cItem.blinkBeforeDestroy)
				cItem.blinkDuration=EditorGUI.DelayedFloatField(new Rect(startX+spaceX, startY, 40, height), cItem.blinkDuration);
			else
				EditorGUI.LabelField(new Rect(startX+spaceX, startY, 40, height), "-");
			...
			cItem.desp=EditorGUI.DelayedTextField(new Rect(startX, startY+spaceY-3, 270, 150), cItem.desp, style);
		...
		void SelectItem(){
			if(collectibleDB.collectibleList.Count<0) return;
